...
class HabitsEditorReducer : Reducer<HabitsEditorState, HabitsEditorAction> {
    override fun invoke(state: HabitsEditorState, action: HabitsEditorAction): HabitsEditorState =
        when (action) {
            HabitsEditorAction.ResetEditor -> HabitsEditorState.initialState()
            HabitsEditorAction.CreateHabit -> state.copy(isNewHabit = true, habit = Habit.empty())
            is HabitsEditorAction.EditHabit -> state.copy(isNewHabit = false, habit = action.habit)
            is HabitsEditorAction.UpdateHabitData -> state.habit?.let {
                state.copy(habit = action.habitData.modifyHabit(it))
            } ?: state
